...
describe('az-autofocus', () => {
  let $compile, scope, el, node, input, textarea, $timeout;

  const basicTemplate = '<div><input az-autofocus="{{focus}}" id="main-input" refocus /><textarea id="textarea"></textarea></div>';
  ...
  it(`should not refocus the previously focused element when the refocus attribute doesn't exist`, () => {
    compileAndDigest(
      '<div><input az-autofocus="{{focus}}" id="main-input" /><textarea id="textarea"></textarea></div>'
    );
    textarea.focus();
    scope.focus = true;
    scope.$digest();
